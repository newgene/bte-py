...
class MetaKGParser:
    ...
    def get_non_TRAPI_metadatas(self, data, extra_data=None):
        parser = API(data)
        mkg = self.extract_metakgedges(
            parser.metadata["operations"], extra_data=extra_data
        )
    ...
    def get_TRAPI_metadatas(self, data, extra_data=None):
        ...
        for i, metadata in enumerate(metadata_list):
            ops.extend(
                self.get_ops_from_metakg_endpoint(
                    metadata, f"[{i+1}/{count_metadata_list}]"
                )
            )
