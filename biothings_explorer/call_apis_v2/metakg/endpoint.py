...
class Endpoint:
    ...
    def resolve_ref_if_provided(self, rec):
        if rec and "$ref" in rec:
            return self.api_meta_data.get("components").fetch_component_by_ref(
                rec["$ref"]
            )
        return rec
    ...
    def construct_association(self, input, output, op):
        return {
            "input_id": (
                self.remove_bio_link_prefix(input["id"])
                if self.remove_biolink
                else input["id"]
            ),
            "input_type": (
                self.remove_bio_link_prefix(input["semantic"])
                if self.remove_biolink
                else input["semantic"]
            ),
            "output_id": (
                self.remove_bio_link_prefix(output["id"])
                if self.remove_biolink
                else output["id"]
            ),
            "output_type": (
                self.remove_bio_link_prefix(output["semantic"])
                if self.remove_biolink
                else output["semantic"]
            ),
            "predicate": (
                self.remove_bio_link_prefix(op["predicate"])
                if self.remove_biolink
                else op["predicate"]
            ),
    ...
    def construct_response_mapping(self, op):
        ...
        return {
            f"{op['predicate']}": self.resolve_ref_if_provided(
                op.get("response_mapping")
            )
        }

    def parse_individual_operation(self, op, method, path_params):
        res = []
        query_operation = self.construct_query_operation(
            {"op": op, "method": method, "path_params": path_params}
        )
    ...
    def construct_endpoint_info(self):
        ...
        for method in ["get", "post"]:
            if method in self.path_item_object:
                ...
                if "x-bte-kgs-operations" in self.path_item_object[
                    method
                ] and isinstance(
                    ...
                    for rec in self.path_item_object[method]["x-bte-kgs-operations"]:
                        ...
                        for op in operation:
                            ...
                            res = [
                                *res,
                                *self.parse_individual_operation(
                                    op, method, path_params
                                ),
                            ]
        return res
